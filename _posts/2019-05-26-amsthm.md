---
layout: page
title: AMS Theorem Style
author: Mahrud Sayrafi
categories: examples
date: 2019-05-26
---

Proof supports enumerated math environments as in the `amsthm` package, including `definition`, `theorem`, `lemma`, `proof`, and a few more[^fn1]. To declare an environment, simply use this template:
...
[^fn1]:
	The full list is: `theorem lemma problem proposition conjecture corollary definition example exercise remark note`.

Most parts of this template can be skipped or customized, so experiment with them. For instance, to use your own enumeration, use `{:.label*}` instead.


## An Example
{:.label}

The following exposition is taken from [here](https://www.maths.tcd.ie/~dwilkins/LaTeXPrimer/Theorems.html).
